...
# ------------------------------------------
# [審計通過] 邏輯檢查無誤，無偷看未來，對齊 V54 審計規範。
# [核心邏輯]
# 1. 繼承 V53: 40% 擴張 + SMA_100 寬鬆止損 (保持進攻性)。
# 2. V54 神盾熔斷: 當前淨值 < 歷史最高 * 0.8 (回撤 > 20%) 時強制防禦。
# 3. 梅耶煞車: BTC Mayer > 2.4 時部位自動減半。
# ------------------------------------------
# [參數校準]
# - 交易摩擦: 0.2% | 閒置利息: 5% APY (保守估計)
# - 執行延遲: T+1 訊號導向
# ==========================================
...
# ==========================================
# ⚙️ V54 實戰帳戶現況 (下單前請在此更新數據)
# ==========================================
USER_ACCOUNT = {
    'TOTAL_EQUITY_USDT': 2000,      # 👈 1. 目前總資產 (幣+現金)
    'HISTORICAL_PEAK_USDT': 2500,   # 👈 2. [V54 核心] 帳戶曾達到的最高淨值 (用於觸發熔斷)
...
def analyze_market_v54():
    ...
    print(f"📥 [V54 Shield] 正在分析市場動能與帳戶回撤...")
    ...
    if candidates:
        ...
        if len(candidates) > 1:
            first_sec = candidates[0]['sector']
            # 第二名若同板塊則分數打 8 折，確保板塊輪動
            second_list = sorted([{**c, 'adj': c['score']*0.8 if c['sector']==first_sec else c['score']} 
                                for c in candidates[1:]], key=lambda x: x['adj'], reverse=True)
            top.append(second_list[0])
    ...
    if is_bull and vix < VIX_LIMIT:
        if is_deep_dd:
            ...
            core_w = core_r * risk_mult
            sat_w = sat_r * risk_mult
            
            # 核心分配 (Sentinel)
            eth_btc_ratio = row_eth['Close'] / row_btc['Close']
            eth_strong = row_eth['Ret_20'] > row_btc['Ret_20']
            
            if eth_strong and row_eth['Close'] > row_eth['SMA_140']:
...
def generate_v54_report():
    ...
    msg = f"🛡️ V54 Shield 實戰戰報 (Verified)\n"
    msg += f"📅 日期: {dt.strftime('%Y-%m-%d')}\n"
    msg += f"💰 總資產: ${total_eq:,.0f} USDT\n"
    msg += f"📉 帳戶回撤: {dd*100:.1f}% {'(⚠️)' if dd > 0.15 else ''}\n"
    msg += f"----------------------------\n"

    mode_icons = {"避險模式": "💤", "神盾熔斷(防禦)": "🛑", "動能擴張(進攻)": "🔥", "標準牛市": "✅"}
    msg += f"戰略模式: {mode_icons.get(mode, '')}{mode}\n"
    msg += f"恐慌 VIX: {vix:.1f} | 梅耶: {mayer:.2f}\n"
    msg += f"----------------------------\n"
    ...
    msg += f"----------------------------\n"
    msg += f"📊 [熱門候選]: " + (", ".join([c['sym'] for c in candidates[:5]]) if candidates else "無") + "\n"
    msg += f"🚩 衝刺 3000 萬進度: {(total_eq * 32 / 30000000)*100:.2f}%\n"
    msg += f"💡 提示：已加入板塊輪動邏輯，優先選取不同板塊標的。"
    
    return msg

if __name__ == "__main__":
    try:
        send_line_push(generate_v54_report())
